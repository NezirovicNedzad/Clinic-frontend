import { PACIJENT_DELETE_FAIL, PACIJENT_DELETE_REQUEST, PACIJENT_DELETE_SUCCESS, PACIJENT_DETAILS_FAIL, PACIJENT_DETAILS_REQUEST, PACIJENT_DETAILS_SUCCESS, PACIJENT_IZABRANI_LIST_FAIL, PACIJENT_IZABRANI_LIST_REQUEST, PACIJENT_IZABRANI_LIST_SUCCESS, PACIJENT_ODELJENJA_LIST_FAIL, PACIJENT_ODELJENJA_LIST_REQUEST, PACIJENT_ODELJENJA_LIST_SUCCESS, PACIJENT_PREBACI_FAIL, PACIJENT_PREBACI_REQUEST, PACIJENT_PREBACI_SUCCESS } from "../constants/pacijentConstants";
...
export const listPacijentOdeljenja = (id) => async (dispatch,getState) => {
    try {
      ...
      const { data } = await axios.get(`http://localhost:5000/api/Pacijent/odeljenja/${id}`,config);
  ...
  export const izabraniPacijenti = (idLekara) => async (dispatch,getState) => {
    try {
      ...
      const { data } = await axios.get(`http://localhost:5000/api/Pacijent/IzabraniLekar/${idLekara}`,config);
  ...
  export const detailsPatient = (id) => async (dispatch,getState) => {
    try {
      ...
      const { data } = await axios.get(
        `http://localhost:5000/api/Pacijent/${id}`,config
      );
