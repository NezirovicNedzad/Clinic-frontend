...
<button onClick={()=>create()} style={{alignItems:"center",margin:"1rem",background:"green",padding:"10px",color:'white'}}><FontAwesomeIcon  style={{marginRight:"0.6rem"}}   icon={faEdit }  />Kreiraj
</button>


{loading ? <p>loading ...</p>  :  odeljenja.map(odeljenje=>(



<div style={{border:"1px solid black",padding:"20px"}} key={odeljenje.id}>
  <button onClick={()=>selectedOdeljenje(odeljenje.id)} style={{float:"right",background:"green",padding:"10px",color:'white'}}><FontAwesomeIcon  style={{marginRight:"0.6rem"}}   icon={faEdit }  />Izmeni</button>
  <button onClick={()=>deleteOd(odeljenje.id)} style={{float:"right",background:"red",padding:"10px",color:'white'}}><FontAwesomeIcon  style={{marginRight:"0.6rem"}}   icon={faDeleteLeft }  />Izbrisi</button>
<p>Naziv:{odeljenje.naziv}</p>
<p>Broj kreveta:{odeljenje.brojKreveta}  :    Broj pacijenata{odeljenje.brojPacijenata}</p>
