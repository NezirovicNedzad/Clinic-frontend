...
dispatch(OdeljenjeUpdate({id:odeljenje.id,naziv,brojKreveta,brojPacijenata})) ;   
    }
    useEffect(()=>{
...
<input name='naziv' value={naziv}   onChange={(e)=>setNaziv(e.target.value)} type='text'  width="100px"/>
<p style={{fontSize:"20px",margin:"10px"}}>Broj Kreveta</p>
<input  name='brojKreveta' value={brojKreveta }  onChange={(e)=>setBrojKreveta(e.target.value)} type="number"   width="100px"/>
<p style={{fontSize:"20px",margin:"10px"}}>Broj Pacijenata</p>
<input name='brojPacijenata' value={brojPacijenata } onChange={(e)=>setBrojPacijenata(e.target.value)} type="number"    width="100px"/>
<br></br>
<button onClick={Submit} style={{margin:"10px",background:"red",padding:"10px"}} >Izmeni</button>
    </div>
  )
}
